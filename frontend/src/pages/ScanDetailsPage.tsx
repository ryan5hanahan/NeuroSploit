...
import {
  ...
  SkipForward, Check, Minus, Pause, Play, Download, Repeat2, ArrowLeftRight
} from 'lucide-react'
import Card from '../components/common/Card'
import Button from '../components/common/Button'
import { SeverityBadge } from '../components/common/Badge'
import { scansApi, reportsApi, agentTasksApi, agentV2Api, vulnerabilitiesApi, enrichmentApi } from '../services/api'
import { wsService } from '../services/websocket'
import { useScanStore } from '../store'
import type { Endpoint, Vulnerability, WSMessage, ScanAgentTask, Report, AgentV2StatusResponse, AgentV2Finding } from '../types'
...
export default function ScanDetailsPage() {
  ...
  const [agentData, setAgentData] = useState<(AgentV2StatusResponse & { findings?: AgentV2Finding[]; operation_id: string }) | null>(null)
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if ((!vulnsData.vulnerabilities || vulnsData.vulnerabilities.length === 0)) {
          const scanAgent = await agentV2Api.getByScan(scanId)
          if (scanAgent) {
            const [agentStatus, agentFindings] = await Promise.all([
              agentV2Api.getStatus(scanAgent.operation_id),
              agentV2Api.getFindings(scanAgent.operation_id),
            ])
            const fullData = { ...agentStatus, findings: agentFindings.findings }
            setAgentData(fullData)
            // Convert V2 findings to vulnerability format for display
            if (agentFindings.findings && agentFindings.findings.length > 0) {
              const mapV2Finding = (f: AgentV2Finding, idx: number): Vulnerability => ({
                id: `v2-${scanAgent.operation_id}-${idx}`,
                scan_id: scanId,
                title: f.title,
                ...
                affected_endpoint: f.endpoint || null,
                poc_request: null,
                poc_response: null,
                poc_payload: null,
                poc_parameter: null,
                ...
                proof_of_execution: undefined,
                negative_controls: undefined,
                created_at: new Date().toISOString()
              })
              const mappedVulns: Vulnerability[] = agentFindings.findings.map(mapV2Finding)
              setVulnerabilities(mappedVulns)
            }
            // Update scan progress from agent
            if (agentStatus.steps_used !== undefined) {
              const progress = Math.min(Math.round(agentStatus.steps_used / agentStatus.max_steps * 100), 100)
              updateScan(scanId, {
                progress,
                current_phase: agentStatus.status
              })
            }
          }
        }
  ...
  const handleRepeatScan = async () => {
    ...
    setIsRepeating(true)
    try {
      const newScan = await scansApi.repeat(scanId)
      navigate(`/scan/${newScan.id}`)
    } catch (error: any) {
      console.error('Failed to repeat scan:', error)
      addLog('error', `Failed to repeat scan: ${error?.response?.data?.detail || 'Unknown error'}`)
    } finally {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            {currentScan.repeated_from_id && (
              <button
                onClick={() => navigate(`/scan/${currentScan.repeated_from_id}`)}
                className="text-xs text-dark-400 hover:text-primary-400 flex items-center gap-1"
              >
                Repeated from original scan <ExternalLink className="w-3 h-3" />
              </button>
            )}
          </div>
        </div>
        <div className="flex gap-2">
          {agentData?.operation_id && (
            <Button variant="secondary" onClick={() => navigate(`/agent/${agentData.operation_id}`)}>
          ...
          {(['completed', 'stopped', 'failed'].includes(currentScan.status)) && (
            <>
              <Button variant="secondary" onClick={handleRepeatScan} isLoading={isRepeating}>
                <Repeat2 className="w-4 h-4 mr-2" />
                Repeat Scan
              </Button>
              <Button variant="ghost" onClick={() => navigate(`/compare?scan=${scanId}`)}>
