#!/usr/bin/env python3
"""
SploitAI - AI-Powered Penetration Testing Framework
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/sploitai.log'),
...
class Completer:
    def __init__(self, sploitai):
        self.sploitai = sploitai
        self.commands = [
        ...
        self.agent_roles = list(self.sploitai.config.get('agent_roles', {}).keys())
        self.llm_profiles = list(self.sploitai.config.get('llm', {}).get('profiles', {}).keys())
...
class SploitAI:
    """Main framework class for SploitAI"""
    ...
    def __init__(self, config_path: str = "config/config.json"):
        ...
        logger.info(f"SploitAI initialized - Session: {self.session_id}")
    ...
    def experience_mode(self):
        ...
        print("""
        ╔═══════════════════════════════════════════════════════════╗
        ║       SPLOIT.AI - EXPERIENCE MODE (WIZARD)                 ║
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>Generated by <strong>sploit.ai</strong> - AI-Powered Penetration Testing Framework</p>
    ...
    def execute_real_scan(self, target: str, scan_type: str = "full", agent_role: str = None, recon_context: Dict = None) -> Dict:
        ...
        """
        print(f"\n{'='*70}")
        print("    SploitAI - Real Penetration Test Execution")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
...
"""
    ...
    def run_full_recon(self, target: str, with_ai_analysis: bool = True) -> Dict:
        ...
        """
        print(f"\n{'='*70}")
        print("    SPLOIT.AI - FULL ADVANCED RECON")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
        print(f"[*] Session ID: {self.session_id}")
...
"""
    ...
    def run_ai_agent(
        ...
        print(f"\n{'='*70}")
        print("    SPLOIT.AI AUTONOMOUS SECURITY AGENT")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
        if prompt_file:
    ...
    def run_autonomous_agent(
        ...
        print(f"\n{'='*70}")
        print("   SPLOIT.AI AUTONOMOUS AI AGENT")
        print(f"{'='*70}")
        print(f"\n[*] Target: {target}")
...
"""
    ...
    def interactive_mode(self):
        ...
        print("""
        ╔═══════════════════════════════════════════════════════════╗
        ║         SploitAI - AI Offensive Security             ║
        ║                  Interactive Mode                         ║
        ╚═══════════════════════════════════════════════════════════╝
        """)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='SploitAI - AI-Powered Penetration Testing Framework',
...
1. CLI MODE (Direct command-line):
   python sploitai.py --input "Your prompt" -cf context.json --llm-profile PROFILE
...
3. EXPERIENCE/WIZARD MODE (-e):
   python sploitai.py -e
   Guided step-by-step configuration - RECOMMENDED for beginners!

EXAMPLES:
=========
  # Step 1: Run recon to collect data
  python sploitai.py --recon example.com

  # Step 2: LLM-only analysis (no tool execution)
  python sploitai.py --input "Analyze for SQLi and XSS" -cf results/context_X.json --llm-profile claude_opus

  # Or use wizard mode
  python sploitai.py -e

  # Run full pentest scan with tools
  python sploitai.py --scan https://example.com
  ...
  python sploitai.py -i
    ...
    framework = SploitAI(config_path=args.config)
    ...
    # Handle target with context file (AI pentest without recon)
        ...
        print("\n" + "="*70)
        print("QUICK START:")
        print("  1. Install tools:  python sploitai.py --install-tools")
        print("  2. Run scan:       python sploitai.py --scan https://target.com")
        print("  3. Interactive:    python sploitai.py -i")
        print("="*70)
